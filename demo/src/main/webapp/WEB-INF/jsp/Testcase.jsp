...
/**------------------------圖表js------------------------------------------------------------------------------------------------------**/
	var lineChartData = {
		labels: ["testsuite1","testsuite2","testsuite3"], //顯示區間名稱
		    datasets: [{
				label: 'suite數量',
		        lineTension: 0,
		        fill: true,
	...
	<body style="background-color:#f1f1f1">
		...
		<div id="topflow" style="display:flex;width:100%;height:450px; background-color:bisque;margin-bottom:20px;">
			...
			<div style="flex:1;width:50px;">
				<table border='1' style="width:30%;border:3px #FFD382 dashed;margin-left:100px;">
					<p style="margin-left:150px;margin-top:150px;">日期區間為：${rightnow}</p>
					<c:forEach var="checkStatusItem2" items="${suiteData}" varStatus="loop">
						<tr>
							<td style="background-color:#bfa145">Id</td>
							<td style="width:75%;background-color: bisque" class="boxTitle">${checkStatusItem2.id}</td>
						</tr>
						<tr>
							<td style="background-color:#bfa145">Suiteid</td>
							<td style="width:75%;background-color: bisque" class="boxTitle">${checkStatusItem2.usersid}</td>
						</tr>
						<tr>
							<td style="background-color:#bfa145">Name</td>
							<td style="width:75%;background-color: bisque" class="boxTitle">${checkStatusItem2.name}</td>
						</tr>
						<tr>
							<td style="background-color:#bfa145">Createdate</td>
							<td style="width:75%;background-color: bisque" class="boxTitle">${checkStatusItem2.createdate}</td>
						</tr>
						<tr>
							<td style="background-color:#bfa145">description</td>
							<td style="width:75%;background-color: bisque" class="boxTitle">${checkStatusItem2.description}</td>
						</tr>
					</c:forEach>
				</table>
			</div>
		...
		<table border='1' style="width:50%;border:3px #FFD382 dashed;margin-left:50px;">
			...
			<tr>
				<td style="background-color:#bfa145">ID</td>
				<td style="background-color:#bfa145" align="center">套件Id</td>
				<td style="background-color:#bfa145" align="center">案件名稱</td>
				<td align="center" style="background-color:#bfa145">是否開啟</td>
				<td style="background-color:#bfa145">新建日期</td>
				<td style="background-color:#bfa145">描述</td>
				<td align="center" style="background-color:#bfa145">詳細內容</td>
			</tr>
			<c:forEach var="checkStatusItem" items="${caseData}" varStatus="loop">
				<tr>
					<td style="width:1%;background-color: bisque" class="boxTitle">${checkStatusItem.id}</td>
					<td align="center" style="width:6%;background-color: bisque" class="boxTitle">${checkStatusItem.testsuiteid}</td>
					<td align="center" style="width:20%;background-color: bisque" class="boxTitle">${checkStatusItem.name}</td>
					<td align="center" style="width:20%;background-color: bisque" class="boxTitle">${checkStatusItem.enabled}</td>
